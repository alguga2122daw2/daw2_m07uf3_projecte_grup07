		...
		<tbody>
			...
			<tr>
				...
				<td class="text-left">
					...
					<a href="{{ route('lloguers.show', $llog->dni_client.','.$llog->id_apartament)}}" class="btn btn-secondary btn-sm">PDF</a>

					<form action="{{ route('lloguers.destroy', $llog->dni_client.','.$llog->id_apartament)}}" method="post" style="display: inline-block">
						@csrf
						@method('DELETE')
